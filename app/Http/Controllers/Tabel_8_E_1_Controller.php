...
class Tabel_8_E_1_Controller extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = Tabel_8_E_1::all();
            return DataTables::of($data)
                ->addIndexColumn()
                ->addColumn('action', function ($row) {
                    $actionBtn = '
                    <form class="inline-block" action="' . route('tabel-8-e-1.destroy', $row->id) . '" method="POST" onsubmit="return confirm(\'Apakah Anda yakin ingin menghapus data ini?\')">
                        <a href="' . route("tabel-8-e-1.edit", $row->id) . ' " . class="edit btn btn-success btn-sm">
    ...
    {
        return view('tabel-8-e-1.create');
    }
    ...
    public function edit(string $id)
    {
        ...
        return view('tabel-8-e-1.edit', compact('tabel_8_e_1'));
    }
