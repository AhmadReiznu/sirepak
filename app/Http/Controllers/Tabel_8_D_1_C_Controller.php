...
class Tabel_8_D_1_C_Controller extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = Tabel_8_D_1_C::all();
            return DataTables::of($data)
                ->addIndexColumn()
                ->addColumn('action', function ($row) {
                    $actionBtn = '
                    <form class="inline-block" action="' . route('tabel-8-d-1-c.destroy', $row->id) . '" method="POST" onsubmit="return confirm(\'Apakah Anda yakin ingin menghapus data ini?\')">
                        <a href="' . route("tabel-8-d-1-c.edit", $row->id) . ' " . class="edit btn btn-success btn-sm">
        ...
        return view('tabel-8-d-1-c.index');
    }
    ...
    {
        return view('tabel-8-d-1-c.create');
    }
    ...
    public function edit(string $id)
    {
        ...
        return view('tabel-8-d-1-c.edit', compact('tabel_8_d_1_c'));
    }
