...
class Tabel_8_F_4_2_Controller extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = Tabel_8_F_4_2::all();
            return DataTables::of($data)
                ->addIndexColumn()
                ->addColumn('action', function ($row) {
                    $actionBtn = '
                    <form class="inline-block" action="' . route('tabel-8-f-4-2.destroy', $row->id) . '" method="POST" onsubmit="return confirm(\'Apakah Anda yakin ingin menghapus data ini?\')">
                        <a href="' . route("tabel-8-f-4-2.edit", $row->id) . ' " . class="edit btn btn-success btn-sm">
        ...
        return view('tabel-8-F-4-2.index');
    }
    ...
    {
        return view('tabel-8-F-4-2.create');
    }
    ...
    public function edit(string $id)
    {
        ...
        return view('tabel-8-F-4-2.edit', compact('tabel_8_f_4_2'));
    }
